...
\begin{document}
	...
	\label{sec:policySelectionInADynamicEnvironment}
		...
		\label{sec:consideringMultipleObstacles}
			...
			We now attempt to try and reduce this further by similar comparisons. However, lack of knowledge of the values of $ \lambda_B $ and $ \lambda_C $ prevents us from reducing or drawing comparison between these costs further. However, we would highlight it beforehand that the attempt below involves combining 2 inequalities to give an absolute upper bound value which may not represent a policy (or path) that has a meaning in reality.
