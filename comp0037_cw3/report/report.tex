...
\begin{document}
	...
	\label{sec:decisionReplanPolicy}
		...
		\label{sec:policySelectionAtStart}
			...
			\\
			\\
			We can see from Fig. \ref{fig:plannedPathAisleB} that the cost of policy that goes down aisle B, $\pi_{K}^{1}$, can be given by the expression in Eq. \ref{eqn:waitPolicyCostStart}. Once again the quantity T represents the amount of time the robot must wait and the quantity $L_W$ represents the cost of waiting one unit of time. In this case there is only one nature action to be considered. This is the blocked aisle B.
			...
			Similarly the cost of the policy that goes down aisle C, $\pi_{K}^{2}$, is given by the expression in Eq. \ref{eqn:aisleCPolicyCostStart}.
			...
			As we are concerned with the average case scenario we compare the expected value of the loss function just as we did in \S \ref{sec:policySelectionWhenObstacleIsObserved}. $\pi_{K}^{2}$ will be chosen over $\pi_{K}^{1}$ if the expected value of it's cost is lower as shown by Eq. \ref{eqn:expectationInequality}.
			...
			Substituting the loss functions from Eq. \ref{eqn:waitPolicyCostStart} and \ref{eqn:aisleCPolicyCostStart} into the inequality in Eq. \ref{eqn:expectationInequality} we obtain the result shown in Eq. \ref{eqn:expectationInequality1}.
