		...
		\subsection{Considering Multiple Obstacles}
			...
			\begin{itemize}
				...
				\item Policy $\pi_{K}^{3}$ : The robot goes down aisle B and observes an obstacle so re-plans down aisle C. While travelling down aisle C it observes another obstacles so it re-plans down aisle D which is known to have no obstacles. The expected cost for this policy is given by:
					\begin{equation}
						\mathbb{E}\left[L\left(\pi_{K}^{3}\right)\right] = L_{IB_1} + L_{B_1C_1} + L_{C_1} + L_{DG}
				...
				\item Policy $\pi_{K}^{5}$ : The robot goes down aisle C and observes an obstacle so it re-plans down aisle D which is known to have no obstacles. The expected cost for this policy is given by:
					\begin{equation}
						\mathbb{E}\left[L\left(\pi_{K}^{5}\right)\right] = L_{IC_1} + L_{C_1D} + L_{DG}
				...
				\item Policy $\pi_{K}^{6}$ : The robot directly goes down aisle D which is known to have no obstacles and avoids both aisles B and C. The expected cost for this policy is given by:
					\begin{equation}
						\mathbb{E}\left[L\left(\pi_{K}^{6}\right)\right] = L_{ID} + L_{DG}
			...
			\begin{equation}
				 \mathbb{E}\left[L\left(\pi_{K}^{6}\right)\right] \leq L_{IB_1} + \mathbb{E}_{B}\left[T\right]L_W + L_{B_1B} + L_{BC} + L_{CG}
			\label{eqn:policyOneInequlaity}
			\end{equation}
			
			Similarly to choose $\pi_{K}^{6}$ over $\pi_{K}^{2}$ the inequality in Eq. (\ref{eqn:policyTwoInequlaity}) must hold:
			
			\begin{equation}
				 \mathbb{E}\left[L\left(\pi_{K}^{6}\right)\right] \leq L_{IB_1} + L_{B_1C_1} + \mathbb{E}_{C}\left[T\right]L_W + L_{C_1C} + L_{CG}
			\label{eqn:policyTwoInequlaity}
			\end{equation}
			
			by comparing the two inequalities in Eq. (\ref{eqn:policyOneInequlaity}) and Eq. (\ref{eqn:policyTwoInequlaity}) we can see that even though there are some terms that are common, it is difficult to merge the two into one inequality. However, by using logic, we may derive an upper bound for the path cost of $\pi_{K}^{6}$. From Eq. (\ref{eqn:policyOneInequlaity}) and Eq. (\ref{eqn:policyTwoInequlaity}) we can obtain the following inequality which will also hold:
			
			\begin{equation}
				\mathbb{E}\left[L\left(\pi_{K}^{6}\right)\right] \leq L_{IB_1} + \mathbb{E}_{B}\left[T\right]L_W + L_{B_1B} + L_{BC} + L_{B_1C_1} + \mathbb{E}_{C}\left[T\right]L_W + L_{C_1C} + L_{CG}
			\end{equation}
			
			Here we use the fact that if $A<B+C$ is true and $A<B+D$ is true then it must also follow that $A<B+C+D$. We can use this idea to compile the inequalities comparing each of the 5 policies to $\pi_{K}^{6}$ into one single inequality:
			
			\begin{equation}
				\mathbb{E}\left[L\left(\pi_{K}^{6}\right)\right] \leq L_{IB_1} + \mathbb{E}_{B}\left[T\right]L_W + L_{B_1B} + L_{BC} + L_{CG} + L_{B_1C_1} + \mathbb{E}_{C}\left[T\right]L_W + L_{C_1C} + L{C_1D} + L_{DG} + L_{IC_1} + L_{ID}
			\end{equation}
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
